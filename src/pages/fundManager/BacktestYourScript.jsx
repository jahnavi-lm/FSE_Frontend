...
// setForm((prev) => ({ ...prev, symbolList: [...prev.symbolList, symbol] }));
// }
// }
// setDropdownVisible(false);
// };
// const removeSymbol = (symbol) => {
// setForm((prev) => ({ ...prev, symbolList: prev.symbolList.filter((s) => s !== symbol) }));
...
// <div
// key={symbol}
// className={`px-4 py-2 flex justify-between hover:bg-blue-100 cursor-pointer ${selectedIndex ? "text-gray-400 cursor-not-allowed" : ""}`}
    ...
    try {
      ...
      for (const [symbol, candles] of Object.entries(candleResponse.data)) {
        const formatted = Array.from(
          new Map(
            candles.map((d) => [d.x, { x: new Date(d.x), y: d.y }])
          ).values()
        );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="relative">
                ...
                {dropdownVisible && (
                  <div className="absolute z-10 w-full bg-white border mt-1 rounded shadow-lg max-h-60 overflow-y-auto">
                    <div className="px-2 py-1 text-gray-600 font-semibold">
                      Indices
                    </div>
                    {INDICES.map((symbol) => (
                      <div
                    ...
                    <div className="px-2 py-1 text-gray-600 font-semibold mt-2">
                      Stocks
                    </div>
                    {COMPANIES.map((symbol) => (
                      <div
                        ...
                        onClick={() =>
                          !selectedIndex && handleSelectSymbol(symbol)
                        }
                      >
                        <span>{symbol}</span>
                        {form.symbolList.includes(symbol) && <span>✅</span>}
